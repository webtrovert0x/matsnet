...
export default function DomainSearch({
  ...
  const handleSearch = async (e) => {
    ...
    const domainToSearch = searchTerm.endsWith(".poor")
      ? searchTerm
      : `${searchTerm}.poor`;
    ...
    try {
      if (!provider) {
        ...
        if (!account) {
          showToast(
            "Please connect your wallet to search for domains.",
            "error",
          );
  ...
  const domainDisplay = searchTerm.endsWith(".poor")
    ? searchTerm
    : searchTerm
      ? `${searchTerm}.poor`
      : "";
  ...
  return (
    <div
      className="flex-center delay-3 animate-fade-in"
      style={{ flexDirection: "column", width: "100%" }}
    >
      <div
        className="glass-panel search-card"
        style={{
          width: "100%",
          maxWidth: "650px",
          padding: "32px",
      ...
      >
        ...
        {searchResult && (
          ...
          >
            <div
              className="flex-between result-inner"
              style={{ alignItems: "flex-start" }}
            >
              <div>
                <h3 style={{ fontSize: "24px", marginBottom: "8px" }}>
                  {searchResult.domain || domainDisplay}
                </h3>
                {searchResult.available ? (
                  ...
                  <div className="badge badge-taken">
              ...
              <div style={{ textAlign: "right" }}>
                {searchResult.available ? (
                  ...
                  >
                    ...
                    >
                      {searchResult.owner.substring(0, 6)}...
                      {searchResult.owner.substring(
                        searchResult.owner.length - 4,
                      )}
                    </span>
                  </div>
                )}
